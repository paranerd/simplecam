...
class NoiseDetector(threading.Thread):
	def __init__(self):
		...
		self.RATE = int(os.getenv('AUDIO_SAMPLE_RATE')) # Hz, so samples (bytes) per second, e.g. 44100 or 48000
		self.CHUNK_SIZE = int(os.getenv('AUDIO_CHUNK_SIZE')) # How many bytes to read from mic each time (stream.read()), e.g. 512 or 2048
		self.CHUNKS_PER_SEC = math.floor(self.RATE / self.CHUNK_SIZE) # How many chunks make a second? (example: 16.000 bytes/s, each chunk is 1.024 bytes, so 1s is 15 chunks)
		self.CHANNELS = int(os.getenv('AUDIO_CHANNELS'))
		self.OBSERVER_LENGTH = 5 # Time in seconds to be observed after noise
		self.HISTORY_LENGTH = 0 # Seconds of audio cache for prepending to records to prevent chopped phrases (history length + observer length = min record length)
