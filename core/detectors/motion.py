...
class MotionDetector(threading.Thread):
	def __init__(self):
		...
		self.OBSERVER_LENGTH = int(os.getenv('OBSERVER_LENGTH')) # Time in seconds to be observed for motion
		self.threshold = 3 #float(os.getenv('MOTION_THRESHOLD'))

		self.do_add_contours = int(os.getenv('ADD_CONTOURS'))
		self.enable_motion_detection = int(os.getenv('VISUAL_MOTION_DETECTION'))
		self.detect_faces = int(os.getenv('FACE_DETECTION'))
		self.show_image = int(os.getenv('SHOW_IMAGE'))

		self.source = self.init_camera() #cv2.VideoCapture(int(os.getenv('CAMERA')))
		self.codec = cv2.VideoWriter_fourcc('M','J', 'P', 'G') # (*'X264')
		self.height, self.width = self.get_dimensions(self.source)

		self.face_cascade = cv2.CascadeClassifier(os.path.join(CONFIG_PATH, 'haarcascade_frontalface_default.xml'))
	...
	def start_recording(self, path):
		...
		# Set path and FPS
		#self.writer = cv2.VideoWriter('{}.avi'.format(path), self.codec, self.fps, (self.width, self.height))
	...
	def save(self, fps):
		self.writer = cv2.VideoWriter('{}.avi'.format(self.path), self.codec, fps, (self.width, self.height))
		...
		while True:
			...
			if self.enable_motion_detection:
				...
				if self.do_add_contours:
					current_frame, targets = self.add_contours(current_frame, dilated_frame)

				# Set blurred frame as new previous frame
				previous_frame = frame_blur

				if self.do_add_contours:
					current_frame, _ = self.add_contours(current_frame, dilated_frame)
